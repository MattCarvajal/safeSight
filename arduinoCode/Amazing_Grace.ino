...
const float DECEL_THRESHOLD = -2.5;
...
void setup() {
  ...
  if (radar.setDetectThres(100, 2000, 10)) {
...
void loop() {
  ...
  if ((radarValue < 3) && (targetNumber >= 1)){
    pedDetected = true;
    Serial.println("Pedestrian Detected"); 
    Serial.print("target number is "); Serial.println(targetNumber);
  }else{
    pedDetected = false;
  ...
  float accelValue = a.acceleration.y;
  Serial.print("Accelration_Y:"); Serial.println(a.acceleration.y);
